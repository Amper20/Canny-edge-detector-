...
// int max_thread[THREADS_NUM];
// int min_thread[THREADS_NUM];
int *max_thread;
int *min_thread;

void *threadFunction(void *var)
{
	const pixel_t *in = g_in;
	pixel_t *out = g_out;
	const float *kernel = g_kernel;
	const int nx = g_nx;
	const int ny = g_ny;
	const int kn = g_kn;
	const bool normalize = g_normalize;
	...
	int for_size = nx - khalf - khalf + 1;
	int start = thread_id * ceil((double)for_size / thread_num);
	int end = fmin((thread_id + 1) * ceil((double)for_size / thread_num), for_size);

	if (normalize)
	{
		for (m = khalf + start; m < end; m++)
		// for (m = khalf; m < nx - khalf; m++)
		{
			for (n = khalf; n < ny - khalf; n++)
			{
				...
				{
					min = pixel;
					min_thread[thread_id] = pixel;
				}
				if (pixel > max)
				{
					max = pixel;
					max_thread[thread_id] = pixel;
				}
			}
		}
	}
...
int main(const int argc, const char **const argv)
{
	...
	max_thread = malloc(thread_num * sizeof(int));
	min_thread = malloc(thread_num * sizeof(int));
