		  ...
		  bitmap_info_header_t *bitmapInfoHeader)
{
	...
	int start_row = ceil(bitmapInfoHeader->height/numtasks)*taskid - mpi_pad;
	int stop_row = ceil(bitmapInfoHeader->height/numtasks)*(taskid + 1) + mpi_pad;
	
	if(start_row < 0)
		start_row = 0;
	if(stop_row > bitmapInfoHeader->height)
		stop_row = bitmapInfoHeader->height;

	for (i_st = 0; i_st < bitmapInfoHeader->height; i_st++)
	{
		for (j_st = 0; j_st < bitmapInfoHeader->width; j_st++)
		{
			...
			if ( i_st < stop_row && i_st >= start_row)
	      ...
	      const pixel_t *data)
{
	...
	// Palette
	size_t i_st;
	for (i_st = 0; i_st < (1U << bmp_ih->bitspp); i_st++)
	{
		const rgb_t color = {(uint8_t)i_st, (uint8_t)i_st, (uint8_t)i_st};
	...
	// size_t i_st, j_st;
	size_t j_st;
	for (i_st = 0; i_st < bmp_ih->height; i_st++)
	{
		for (j_st = 0; j_st < bmp_ih->width; j_st++)
		{
			c = (unsigned char)data[j_st + bmp_ih->width * i_st];
		 ...
		 const bool normalize)
{
	...
	int start_row = ceil(ny/numtasks)*taskid - khalf;
	int stop_row = ceil(ny/numtasks)*(taskid + 1) + khalf;
...
{	
	...
	for (i = 0; i < 1; i++)
	{
		...
		sprintf(out_file_name, "frames_out/%dout.bmp", i, taskid);

		static bitmap_info_header_t ih;
		pixel_t *in_bitmap_data = load_bmp(file_name, &ih);
		...
		if(taskid == 0){
			for(int j = 1; j < numtasks; j++){
				MPI_Recv(in_bitmap_data, ih.bmp_bytesz, MPI_UNSIGNED_SHORT, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

				int start_row = ceil(ih.height/numtasks)*j;
				int stop_row = ceil(ih.height/numtasks)*(j + 1);
				
				printf("%d %d %d\n", j, start_row, stop_row);

				if(start_row < 0)
					start_row = 0;
				
				if(stop_row > ih.height)
					stop_row = ih.height;

				const int nx = ih.width;
				const int ny = ih.height;
				for (int m = 0; m < nx; m++)
					for (int n = 0; n < ny; n++)
					{
						if(n >= start_row && n <= stop_row) {
							out_bitmap_data[n * nx + m ] = in_bitmap_data[n * nx + m ];
						}
					}
			}
			...
			MPI_Send(out_bitmap_data, ih.bmp_bytesz, MPI_UNSIGNED_SHORT, 0, 0, MPI_COMM_WORLD);
		}
		sprintf(out_file_name, "frames_out/out.bmp");
